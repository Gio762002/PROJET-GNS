...
def set_BGP(lst_as, loopback_plan):# loopback_plan: result of fctr.distribute_loopback(lst_as)
    for AS in lst_as:
        for router in AS.routers.values(): #Cisco: router bgp %as_id， 
            #'' bgp router-id %router_id, 
            #'' no bgp default ipv4-unicast,
            #'' bgp log-neighbor-changes
            for interface in router.interfaces.values():
                if interface.protocol_type == "eBGP":
                    #fctr.find_neighbour_info(interface.connected_router, lst_as)
                    pass
                    #Cisco: neighbor %interface remote-as %neighbour_as_id
                else:
